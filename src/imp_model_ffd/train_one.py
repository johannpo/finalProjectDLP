...
parser = argparse.ArgumentParser(description="Improved FNO with differential local branch, one-step")
...
path_model = config.get_path_model(args.out_dir, step_override=STEP)
path_train_err = config.get_path_train_err(args.out_dir, step_override=STEP)
path_test_err = config.get_path_test_err(args.out_dir, step_override=STEP)
path_test_data = config.get_path_test_data(args.out_dir, step_override=STEP)
...
for ep in range(config.EPOCHS):
    ...
    for xx, yy in train_loader:
        ...
        for t in range(T):
            ...
            xx = torch.cat(
                (xx[..., 1:-2], im, gridx_batch, gridy_batch),
                dim=-1,
            )
...
print("Differential-local one-step training complete.")
