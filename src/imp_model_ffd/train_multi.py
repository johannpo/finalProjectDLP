...
from model_components import ImprovedFNODiffLocal2d
...
parser = argparse.ArgumentParser(description="Improved FNO with differential local branch, multistep")
...
path_model = config.get_path_model(args.out_dir, step_override=config.STEP)
path_train_err = config.get_path_train_err(args.out_dir, step_override=config.STEP)
path_test_err = config.get_path_test_err(args.out_dir, step_override=config.STEP)
path_test_data = config.get_path_test_data(args.out_dir, step_override=config.STEP)
...
print("  STEP   =", STEP)
print("  modes  =", modes)
print("  Run    =", config.get_run_name(STEP))

gridx, gridy = make_grids(S)

train_a = torch.cat(
    (train_hist, gridx.repeat(ntrain, 1, 1, 1), gridy.repeat(ntrain, 1, 1, 1)),
    dim=-1,
)
test_a = torch.cat(
    (test_hist, gridx.repeat(ntest, 1, 1, 1), gridy.repeat(ntest, 1, 1, 1)),
    dim=-1,
)
...
model = ImprovedFNODiffLocal2d(modes, config.WIDTH, T_in, STEP).to(device)
...
for ep in range(config.EPOCHS):
    ...
    for xx, yy in train_loader:
        ...
        for t in range(0, T, STEP):
            ...
            xx = torch.cat(
                (xx[..., STEP:-2], im, gridx_batch, gridy_batch),
                dim=-1,
            )
...
print("Differential-local multistep training complete.")
