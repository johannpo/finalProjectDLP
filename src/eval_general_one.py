...
SRC_DIR = Path("/scratch/s4099265/dlp_project/final_project/src/imp_model_3x3_conv")

MODEL_PATH = Path(
    "/scratch/s4099265/dlp_project/final_project/output/improved_one_3x3/model/improved_one_3x3_samp1000_ep500_m12_w20_tin10_tout40_step1.pt"
)

MODEL_CLASS_NAME = "ImprovedFNO2dMultiStepNoPadding"

DESIRED_MODES_OVERRIDE = None
WIDTH_OVERRIDE = None
EPOCHS_OVERRIDE = None

TRAIN_STDOUT_LOG_PATH = Path(
    "/scratch/s4099265/dlp_project/final_project/logs/imp_model_3x3.sh_28279901.out"
)
...
def infer_step_from_run_name(run_name):
    m = re.search(r"_step(\d+)$", run_name)
    if m is not None:
        return int(m.group(1))
    return 1


def build_model(ModelClass, modes, width, t_in, step):
    sig = inspect.signature(ModelClass.__init__)
    params = [p for p in sig.parameters.values() if p.name != "self"]

    if len(params) == 4:
        return ModelClass(modes, width, t_in, step)
    if len(params) == 3:
        return ModelClass(modes, width, t_in)

    raise TypeError(
        f"Unsupported constructor for {ModelClass.__name__}: "
        f"expected 3 or 4 args after self, got {len(params)}"
    )
...
if not hasattr(model_components, MODEL_CLASS_NAME):
    available = [name for name in dir(model_components) if not name.startswith("_")]
...
if __name__ == "__main__":
    ...
    T = int(data_pack["T"])
    STEP = int(data_pack["STEP"]) if "STEP" in data_pack else infer_step_from_run_name(RUN_NAME)
    ...
    model = build_model(ModelClass, modes, WIDTH, T_in, STEP).to(device)
