...
from model_components import BaselineFNO2dMultiStep
...
parser = argparse.ArgumentParser(description="Baseline FNO 2D multi-step autoregressive training")
...
model = BaselineFNO2dMultiStep(modes, config.WIDTH, T_in, STEP).to(device)
...
for ep in range(config.EPOCHS):
    ...
    for xx, yy in train_loader:
        ...
        for t in range(0, T, STEP):
            ...
            # drop oldest STEP frames, append new STEP predictions, then append x,y
            xx = torch.cat(
                (
                    xx[..., STEP:-2],
                    im,
...
print("Baseline multi-step autoregressive training complete.")
